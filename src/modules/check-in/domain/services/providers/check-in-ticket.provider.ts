...
export class CheckInTicketProvider {
  ...
  public async execute({ code }: { code: string }): Promise<CheckInPayload> {
    ...
    if (ticket.status === ETicketStatus.USED) {
      return {
        ...
        message: `Entrada negada: ingresso já utilizado em ${ticket.usedAt?.toLocaleString('pt-BR')}`,
        ticket,
      };
    }
    ...
    if (ticket.status === ETicketStatus.CANCELLED) {
      return {
        ...
        message: 'Entrada negada: ingresso cancelado',
        ticket,
      };
    }
    ...
    const updated = await this.ticketRepository.manager.save(ticket);

    return {
      result: ECheckInResult.SUCCESS,
      message: 'Entrada liberada! Check-in realizado com sucesso.',
      ticket: updated,
    };
  }
